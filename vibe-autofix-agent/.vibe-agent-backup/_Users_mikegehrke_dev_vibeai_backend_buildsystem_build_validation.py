...
async def validate_build_request(user: str, project_path: str, build_type: str) -> Dict[str, any]:
    """
    ...
    if build_type not in supported_types:
        errors.append(f"Unsupported build type: {build_type}. " f"Supported: {', '.join(supported_types)}")
    ...
    if disk_space_gb < 1.0:
        errors.append(f"Insufficient disk space: {disk_space_gb:.2f} GB available. " "Need at least 1 GB.")
    elif disk_space_gb < 2.0:
        warnings.append(f"Low disk space: {disk_space_gb:.2f} GB available.")

    # 5) Maximale Builds pro User prüfen
    user_builds = get_active_builds_for_user(user)

    if len(user_builds) >= 5:
        errors.append(f"Too many active builds for user {user}. " f"Maximum: 5, Current: {len(user_builds)}")
...
async def _validate_electron_project(project_path: str) -> List[str]:
    ...
    if not os.path.exists(main_js) and not os.path.exists(index_js):
        errors.append("Missing main.js or index.js - " "not a valid Electron project")
...
def get_build_queue_position(build_id: str) -> Optional[int]:
    ...
    for bid, build_data in build_manager.builds.items():
        if build_data.get("status") == "PENDING":
            pending_builds.append({"id": bid, "timestamp": build_data.get("timestamp", 0)})
...
async def check_build_dependencies(build_type: str) -> Dict[str, any]:
    ...
    return {"available": len(missing) == 0, "missing": missing, "version": version}
...
def format_build_error(error: Exception, build_type: str) -> str:
    ...
    if "pubspec.yaml" in error_msg.lower():
        return "Flutter project error: Missing or invalid pubspec.yaml. " "Please check your Flutter project structure."

    elif "package.json" in error_msg.lower():
        return "Node.js project error: Missing or invalid package.json. " "Please check your project configuration."

    elif "permission denied" in error_msg.lower():
        return "Permission error: Unable to access project files. " "Please check file permissions."

    elif "no space left" in error_msg.lower():
        return "Disk space error: Not enough storage available. " "Please free up disk space and try again."

    elif "timeout" in error_msg.lower():
        return "Build timeout: The build took too long to complete. " "Please check your project size and complexity."
