...
async def get_current_user_v2(credentials=Depends(security), db=Depends(get_db)):
    """
...
def create_api_key(user_id: int, name: str = "default") -> str:
    ...
    """
    random_part = hashlib.sha256(f"{user_id}{name}{time.time()}".encode()).hexdigest()[:32]
...
def create_session_token(user_id: int, device_id: Optional[str] = None, ip_address: Optional[str] = None) -> str:
    """
...
def verify_session_token(token: str, device_id: Optional[str] = None, ip_address: Optional[str] = None) -> Dict:
    """
...
def check_rate_limit(identifier: str, max_attempts: int = 5, window_seconds: int = 300):
    ...
    login_attempts[identifier] = [attempt for attempt in login_attempts[identifier] if now - attempt < window_seconds]
...
def verify_oauth_token(provider: str, token: str) -> Optional[Dict]:
    ...
    if provider == "google":
        ...
        try:
            ...
            resp = httpx.get(f"https://oauth2.googleapis.com/tokeninfo?id_token={token}", timeout=5)
...
async def require_developer(current_user=Depends(get_current_user_v2)):
    ...
    if not getattr(current_user, "is_developer", False):
        raise HTTPException(status_code=403, detail="Developer privileges required")
    return current_user
...
def create_user_with_password(username: str, email: str, password: str, db) -> User:
    """
    Erstellt neuen User mit gehashtem Passwort.
    """
    hashed = hash_password(password)

    new_user = User(username=username, email=email, password=hashed, created_at=datetime.utcnow())
...
async def register(request: RegisterRequest, db=Depends(get_db)):
    ...
    # Create user
    user = create_user_with_password(request.username, request.email, request.password, db)
