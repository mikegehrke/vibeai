...
"""
    ...
    async def _create_vue_project(self, path: Path, name: str, options: Optional[Dict]) -> Dict:
        """
...
"""
        with open(path / "src" / "App.vue", "w", encoding="utf-8") as f:
            f.write(app_vue)
        files_created += 1
...
"""
        with open(path / "src" / "main.js", "w", encoding="utf-8") as f:
            f.write(main_js)
        files_created += 1
...
"""
    ...
    async def _create_node_project(self, path: Path, name: str, options: Optional[Dict]) -> Dict:
...
"""
        with open(path / "src" / "index.js", "w", encoding="utf-8") as f:
            f.write(index_js)
        files_created += 1
    ...
    def _init_git(self, path: Path) -> bool:
        ...
        try:
            result = subprocess.run(["git", "init"], cwd=path, capture_output=True, text=True, timeout=10)
    ...
    async def _install_dependencies(self, path: Path, framework: str) -> bool:
        """
        ...
        try:
            if framework == "flutter":
                ...
                result = subprocess.run(["npm", "install"], cwd=path, capture_output=True, timeout=180)
    ...
    def get_stats(self) -> Dict:
        ...
        total_size = sum(self._get_dir_size(self.output_dir / p["project_id"]) for p in projects)
