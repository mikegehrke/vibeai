...
class PreviewRuntime:
    ...
    # FILE WATCHER
    # ---------------------------------------------------------
    async def watch_project(self, user: str, project_id: str, project_path: str, extensions: Set[str] = None):
        """
        ...
        watcher_task = asyncio.create_task(self._watch_loop(user, project_path, file_hashes, extensions))
    ...
    def _scan_files(self, project_path: str, extensions: Set[str]) -> Dict[str, str]:
        """
    ...
    # COMPILATION CACHE
    # ---------------------------------------------------------
    def get_cached_compilation(self, file_path: str, file_hash: str) -> Optional[Dict]:
        """
    ...
    def cache_compilation(self, file_path: str, file_hash: str, result: Dict):
        """
        ...
        self.compilation_cache[cache_key] = {"result": result, "timestamp": time.time()}

    # ---------------------------------------------------------
    # ERROR DETECTION
    # ---------------------------------------------------------
    async def check_syntax_errors(self, project_path: str, file_type: str) -> Optional[Dict]:
        """
        ...
        try:
            if file_type == "js" or file_type == "ts":
                ...
                if process.returncode != 0:
                    return {"type": "syntax_error", "message": stderr.decode()}
                ...
                if process.returncode != 0:
                    return {"type": "syntax_error", "message": stdout.decode()}
