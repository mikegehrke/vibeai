...
def diagnose_api_access():
    ...
    try:
        ...
        print(
            f"   🟢 GPT-3.5: {len(gpt35_models)} ({', '.join(gpt35_models[:2])}{'...' if len(gpt35_models) > 2 else ''})"
        )
...
def run_full_diagnostics():
    ...
    return {"openai_detailed": openai_report, "all_providers": multi_report}
    ...
    def diagnose_openai_modern(self) -> Dict:
        ...
        categories = {
            ...
            "GPT-4 Legacy": [
                m
                for m in all_models
                if "gpt-4" in m.lower() and "gpt-4o" not in m.lower() and "gpt-4.1" not in m.lower()
            ],
...
class APIDiagnosticsV2:
    ...
    async def check_openai_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"provider": "openai", "available": False, "error": str(e)}
    ...
    async def check_anthropic_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"provider": "anthropic", "available": False, "error": str(e)}
    ...
    async def check_ollama_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{base_url}/api/tags", timeout=aiohttp.ClientTimeout(total=2)) as resp:
                    if resp.status == 200:
            ...
            return {"provider": "ollama", "available": False, "error": str(e)}
    ...
    async def check_github_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"provider": "github", "available": False, "error": str(e)}
    ...
    async def health_check(self, use_cache: bool = True) -> Dict[str, Any]:
        ...
        active_providers = sum(1 for p in providers.values() if p.get("available", False))
        total_models = sum(len(p.get("models", [])) for p in providers.values() if p.get("available", False))
