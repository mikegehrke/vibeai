...
async def run_agent_v2(agent_name: str, message: str, context: dict, db: Session = None, user=None):
    """
    Erweiterte Version:
    - Multi-Agent Routing
...
class AgentManager:
    ...
    def _load_agents(self):
        ...
        """
        agents_path = os.path.join(os.path.dirname(__file__), "ai_agents")
    ...
    def list_agents(self) -> List[Dict]:
        ...
        """
        return [{"name": name, **metadata} for name, metadata in self.agent_metadata.items()]
    ...
    def add_to_memory(self, agent_name: str, message: Dict):
        ...
        self.agent_memory[agent_name].append({"timestamp": datetime.utcnow().isoformat(), "message": message})
    ...
    async def run(
        ...
        try:
            ...
            # Update memory
            if save_to_memory:
                self.add_to_memory(actual_name, {"role": "user", "content": message})
                self.add_to_memory(actual_name, {"role": "assistant", "content": response_text})
...
async def run_agent_advanced(agent_name: str, message: str, context: Dict = None, user=None, db=None) -> Dict:
    """
    Convenience function for running agents.
    """
    return await agent_manager.run(agent_name=agent_name, message=message, context=context, user=user, db=db)
