...
from .zip_export import cleanup_temp_zip, create_temp_zip
...
async def download_all_artifacts(build_id: str):
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=build_{build_id}.zip"},
        )
...
async def download_logs(build_id: str):
    ...
    if not os.path.exists(log_path):
        ...
        if not logs:
            raise HTTPException(status_code=404, detail="No logs found for this build")
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=build_{build_id}.log"},
        )

    return FileResponse(path=log_path, media_type="text/plain", filename=f"build_{build_id}.log")
