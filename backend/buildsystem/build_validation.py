...
async def validate_build_request(user: str, project_path: str, build_type: str) -> Dict[str, any]:
    ...
    if build_type not in supported_types:
        errors.append(f"Unsupported build type: {build_type}. Supported: {', '.join(supported_types)}")
    ...
    if len(user_builds) >= 5:
        errors.append(f"Too many active builds for user {user}. Maximum: 5, Current: {len(user_builds)}")
