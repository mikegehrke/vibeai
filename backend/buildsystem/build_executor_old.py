...
class FlutterAndroidExecutor(BuildExecutor):
    ...
    async def execute(
        ...
        Build Android APK.
        """
        await build_manager.add_log(build_id, "🔨 Starting Flutter Android build...")
        ...
        if os.path.exists(apk_path):
            ...
            await build_manager.add_artifact(build_id, "apk", output_apk, apk_size)

            await build_manager.add_log(build_id, f"✅ APK built: {apk_size / 1024 / 1024:.2f} MB")
...
class FlutterIOSExecutor(BuildExecutor):
    ...
    async def execute(
        ...
        Build iOS IPA.
        """
        await build_manager.add_log(build_id, "🔨 Starting Flutter iOS build...")
        ...
        await build_manager.add_log(build_id, "✅ iOS build completed")
...
class FlutterWebExecutor(BuildExecutor):
    ...
    async def execute(
        ...
        Build Flutter Web.
        """
        await build_manager.add_log(build_id, "🔨 Starting Flutter Web build...")
        ...
        await build_manager.add_log(build_id, f"✅ Web build completed: {output_dir}")
...
class ReactWebExecutor(BuildExecutor):
    ...
    async def execute(
        ...
        """
        await build_manager.add_log(build_id, "🔨 Installing dependencies...")
        ...
        await build_manager.add_log(build_id, "🔨 Building React app...")
        ...
        await build_manager.add_log(build_id, f"✅ React build completed: {output_dir}")
...
class NextJSWebExecutor(BuildExecutor):
    ...
    async def execute(
        ...
        await build_manager.add_log(build_id, "🔨 Building Next.js app...")
        ...
        await build_manager.add_log(build_id, "✅ Next.js build completed")
...
async def start_build(
    ...
    try:
        ...
        await build_manager.add_log(build_id, f"🚀 Starting {build_type} build...")
        ...
        result = await executor.execute(
            ...
            config=config or {},
            log_callback=lambda msg: build_manager.add_log(build_id, msg),
        )

        # Update status to SUCCESS
        build_manager.update_build_status(user, build_id, "SUCCESS")

        await build_manager.add_log(build_id, "✅ Build completed successfully")
        ...
        await build_manager.add_log(build_id, f"❌ Build failed: {str(e)}")
