...
class BuilderPipeline:
    ...
    async def build_project(
        ...
        logs.append(structured_output.create_generation_log(
        ...
        for file_node in all_files:
            ...
            # Formatiere Code
            formatted_content = code_formatter.format_code(
                content=content,
                file_path=file_node.path
            )
    ...
    async def update_file(
        ...
        # Formatieren
        formatted = code_formatter.format_code(
    ...
    def _generate_configs(self, project_name: str, project_type: str) -> Dict[str, str]:
        ...
        if project_type == "flutter":
            configs["pubspec.yaml"] = config_writer.generate_pubspec_yaml(
                project_name=project_name
            )
            ...
            configs["tsconfig.json"] = config_writer.generate_tsconfig_json(
                "react-native"
            )
            configs[".gitignore"] = config_writer.generate_gitignore(
                "react-native"
            )
            ...
            configs["requirements.txt"] = config_writer.generate_requirements_txt(
                framework="fastapi"
            )
