...
class FileMerger:
    ...
    def _smart_merge(self, original: str, new: str, language: str) -> Dict:
        ...
        return {"merged_content": merged, "conflicts": conflicts, "changes": changes}
    ...
    def _merge_imports(self, original: str, new: str, language: str) -> str:
        ...
        if language == "python":
            ...
            new_imports = set(re.findall(r"^(?:from .+ )?import .+$", new, re.MULTILINE))
            ...
            # JS/TS imports
            orig_imports = set(re.findall(r"^import .+;?$", original, re.MULTILINE))
            new_imports = set(re.findall(r"^import .+;?$", new, re.MULTILINE))
    ...
    def _merge_functions(self, original: str, new: str, language: str) -> Tuple[str, List[str]]:
        """Merged Funktionen (fügt neue hinzu, ersetzt existierende)."""
        changes = []

        if language == "python":
            # Extrahiere Funktionsnamen
            new_functions = re.findall(r"^def (\w+)\(", new, re.MULTILINE)
    ...
    def _merge_classes(self, original: str, new: str, language: str) -> Tuple[str, List[str]]:
        """Merged Klassen."""
        changes = []

        if language == "python":
            new_classes = re.findall(r"^class (\w+)", new, re.MULTILINE)
    ...
    def _detect_conflicts(self, original: str, new: str, language: str) -> List[str]:
        ...
        if original.strip() and new.strip():
            if original != new:
                ...
                if similarity < 0.3:
                    conflicts.append("Files are very different - manual review recommended")
