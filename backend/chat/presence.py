...
class PresenceManager:
    ...
    def get_typing_status(
        ...
        if user_id or session_id:
            return [
                status for key, status in self.typing_status.items()
                if (not user_id or status.get("user_id") == user_id) and
                   (not session_id or status.get("session_id") == session_id)
            ]
    ...
    def record_performance(
        ...
        if len(self.performance_metrics[agent_name][metric_type]) > 100:
            self.performance_metrics[agent_name][metric_type] = \
                self.performance_metrics[agent_name][metric_type][-100:]
    ...
    def cleanup_stale_data(self, max_age_minutes: int = 30):
        ...
        stale_typing = [
            ...
            if datetime.fromisoformat(status["started_at"]) < cutoff
        ]
