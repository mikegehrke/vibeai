...
class WebSocketManagerV2:
    ...
    async def connect(
        ...
        logger.info(
            f"WebSocket connected: user={user_id}, role={role}, "
            f"group={group}, total={len(self.connections)}"
        )
    ...
    # ---------------------------------------------------------
    async def notify_app_builder_status(
        self,
        project_name: str,
        status: str,
        ...
        """
        await self.broadcast_to_group("app_builder", {
    ...
    async def notify_code_studio_event(
        ...
        """
        await self.broadcast_to_group("code_studio", {
    ...
    async def notify_agent_activity(
        self,
        agent_name: str,
        action: str,
    ...
    async def notify_system_alert(
        ...
        """
        # An Admins
        await self.broadcast_to_role("admin", {
            "type": "system_alert",
            "level": level,
            "message": message,
            "details": details,
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self.lock:
            groups_count = {
                name: len(websockets)
                for name, websockets in self.groups.items()
            }
