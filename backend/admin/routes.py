...
try:
    ...
    from models import User, Project, Session as ChatSession
...
async def get_dashboard_stats(
    ...
    total_messages = db.query(
        func.sum(ChatSession.message_count)
    ).scalar() or 0 if ChatSession else 0
...
async def system_health_check(
    ...
    api_providers = {}
    try:
        from core.model_registry_v2 import model_registry
...
async def get_users_overview(
    ...
    for user in users:
        ...
        ).first() if ChatSession else None
...
async def get_project_stats(
    ...
    recent_count = db.query(Project).filter(
...
async def cleanup_stale_websockets(
