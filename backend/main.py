...
from ai.realtime_generator.realtime_routes import router as realtime_gen_router
...
app.include_router(realtime_gen_router)
...
async def root():
    ...
    return {
        ...
        "status": "running",
        "systems": {
            "codestudio": "✅ 9 Languages - Python, JS, TS, React, "
                          "Dart, Swift, Kotlin, Java, C#",
            "buildsystem": "✅ 5 Platforms - Flutter, React, "
                           "Next.js, Node, Electron",
            "builder": "✅ App Builder - Generate complete projects",
            "agents": "✅ AI Agents - Aura, Cora, Devra, Lumi",
            "admin": "✅ Admin Dashboard - Users, Tickets, "
                     "Notifications",
            ...
            try:
                ...
                # ANTHROPIC CLAUDE MODELS
                    ...
                    try:
                        ...
                        response = client.messages.create(
                            model=request.model if request.model.startswith('claude-') else 'claude-sonnet-4-20250514',
                            max_tokens=max_tokens,
                            temperature=temperature,
                            messages=[
                        ...
                        return {
                            ...
                            "cost": (response.usage.input_tokens * 0.003 / 1000) + (response.usage.output_tokens * 0.015 / 1000) if response.usage else 0,
                            "model": request.model,
                            "provider": "anthropic"
                        }
                    except Exception as e:
                ...
                # GOOGLE GEMINI MODELS
                    ...
                    try:
                        ...
                        model = genai.GenerativeModel(request.model if request.model.startswith('gemini-') else 'gemini-2.0-flash-exp')
                ...
                elif any(x in request.model.lower() for x in ['ollama', 'qwen', 'llama3.2', 'llama3', 'mistral', 'codellama', 'deepseek']):
                    try:
                ...
                needs_fallback = ('model_not_found' in error_str or 
...
async def get_available_models():
    ...
    return {
        "models": [
            ...
            "claude-3-5-haiku-20241022", "claude-3-5-opus",
            ...
            "gemini-3", "gemini-3-pro", "gemini-3-ultra",
            "gemini-3-flash", "gemini-3-nano",
            
            # Gemini 2.5 Series
            "gemini-2.5", "gemini-2.5-pro", "gemini-2.5-flash",
            
            # Gemini 2.0 Series
            "gemini-2.0", "gemini-2.0-flash-exp", "gemini-2.0-pro",
            "gemini-2.0-ultra", "gemini-2.0-nano",
            ...
            "github-gemini-3", "github-gemini-2.5", "github-gemini-2.0-flash",
        ...
        "use_cases": {
            "chat": ["gpt-5.1", "gpt-4o", "claude-4.5-opus", "gemini-3-pro", "github-gpt-4.1"],
            "code": ["gpt-5.1-codex", "codex", "github-codex", "codellama:70b", "deepseek-coder-v2", "Phi-4", "qwen2.5-coder:32b"],
            "reasoning": ["o3", "o3-mini", "o1-preview", "claude-4.5-opus", "gemini-3-ultra"],
            "fast": ["gpt-4o-mini", "claude-3.5-haiku", "gemini-2.0-flash-exp", "llama3.2:3b"],
            "local": ["llama3.3:70b", "mixtral:8x22b", "qwen2.5:72b", "deepseek-coder-v2:16b"],
            "vision": ["gpt-4o", "claude-4.5", "gemini-3-pro", "Llama-3.2-90B-Vision-Instruct"],
