...
class FlowController:
    ...
    async def execute_full_flow(
        ...
        try:
            ...
            code_result = await self._execute_code_generation(
                ui_result.get("result"),
                framework
            )
            ...
            project_path = await self._setup_project(
            ...
            preview_result = await self._start_preview(
                project_path,
                framework
            )
            ...
            deploy_result = await self._generate_download(
                build_result.get("result", {}).get("build_id")
            )
    ...
    async def _execute_code_generation(self, ui_structure: Dict, framework: str) -> Dict:
        ...
        return await orchestrator.execute_task(
            task_type=task_type,
            params={"screen": ui_structure.get("screen")}
        )
...
"""
    ...
    async def _generate_download(self, build_id: str) -> Dict:
        ...
        return await orchestrator.execute_task(
            task_type="generate_download_link",
