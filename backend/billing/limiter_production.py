...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.requests: Dict[str, Dict[str, list]] = defaultdict(lambda: {"minute": [], "hour": [], "day": []})
        self.tokens: Dict[str, Dict[str, int]] = defaultdict(lambda: {"day": 0, "month": 0})
        self.costs: Dict[str, Dict[str, float]] = defaultdict(lambda: {"month": 0.0})
        self.builds: Dict[str, Dict[str, int]] = defaultdict(lambda: {"day": 0, "month": 0})
    ...
    def check_request_limit(self, user_id: str, tier: str, window: str = "minute"):
        """
        ...
        if USE_REDIS and redis_client:
            ...
            # In-memory fallback
            self.requests[user_id][window] = [t for t in self.requests[user_id][window] if now - t < window_seconds]
        ...
        try:
            ...
            if WS_AVAILABLE:
                await ws_manager.send_personal_message(
                    user_id, {"type": "rate_limit", "message": e.detail, "tier": tier}
                )
            raise
    ...
    def add_token_usage(self, user_id: str, tier: str, tokens: int):
        """
    ...
    def add_cost(self, user_id: str, tier: str, cost_usd: float):
        """
        ...
        if USE_REDIS and redis_client:
            ...
            if max_cost > 0 and total > max_cost:
                raise HTTPException(
                    status_code=402,
                    detail=f"Monthly cost limit exceeded (${max_cost:.2f}). " f"Current: ${total:.2f}. Please upgrade.",
                )
        else:
    ...
    def check_builder_limit(self, user_id: str, tier: str):
        """
        ...
        if USE_REDIS and redis_client:
            ...
            if self.builds[user_id]["day"] > limits["builder_builds_per_day"]:
                raise HTTPException(status_code=429, detail=f"Daily build limit exceeded")

    # ────────────────────────────────────────────────────────────────────────
    # CONCURRENT SESSIONS
    # ────────────────────────────────────────────────────────────────────────

    def check_concurrent_sessions(self, user_id: str, tier: str):
        """
    ...
    async def enforce(
        ...
        try:
            ...
            logger.info(
                f"[LIMITER OK] user={user_id}, tier={tier}, " f"tokens={tokens_expected}, cost=${cost_estimate:.4f}"
            )
            ...
            logger.warning(f"[LIMITER BLOCKED] user={user_id}, tier={tier}, " f"reason={e.detail}")
            raise
    ...
    def get_quota_status(self, user_id: str, tier: str) -> Dict:
        ...
        if USE_REDIS and redis_client:
            ...
            return {
                "tier": tier,
                "requests": {
                    "minute": len(self.requests[user_id]["minute"]),
                    "hour": len(self.requests[user_id]["hour"]),
                    "day": len(self.requests[user_id]["day"]),
                },
...
def enforce_limits(user, tokens_used=0):
    ...
    if daily_requests >= rules["requests_per_day"]:
        raise HTTPException(429, f"Daily limit reached ({rules['requests_per_day']})")
    ...
    if (monthly_tokens + tokens_used) >= rules["tokens_per_month"]:
        raise HTTPException(402, "Monthly token quota exceeded – please upgrade")
