...
async def capture_order_advanced(
    order_id: str,
    current_user=Depends(get_current_user_v2),
    db=Depends(get_db)
):
    ...
    if len(parts) >= 3:
        ...
        if order_type in ["pro", "ultra"]:
            ...
            subscription = db.query(SubscriptionDB).filter(
                SubscriptionDB.user_id == str(current_user.id)
            ).first()
            ...
            if subscription:
                ...
                subscription.renewal_date = (
                    datetime.utcnow() + timedelta(days=365 if billing_cycle == "yearly" else 30)
                )
        ...
        # 3. Handle Credits
            ...
            user_credits = db.query(UserCreditsDB).filter(
                UserCreditsDB.user_id == str(current_user.id)
            ).first()
