...
async def generate_invite_code(
    ...
    pending_count = db.query(ReferralBonusDB).filter(
        ReferralBonusDB.user_id == user_id,
...
async def use_invite_code(
    ...
    referral = db.query(ReferralBonusDB).filter(
    ...
    existing = db.query(ReferralBonusDB).filter(
    ...
    inviter_credits_db = db.query(UserCreditsDB).filter(
        UserCreditsDB.user_id == inviter_id
    ).first()
    ...
    invitee_credits_db = db.query(UserCreditsDB).filter(
        UserCreditsDB.user_id == new_user_id
    ).first()
...
async def get_my_referrals(
    ...
    referrals = db.query(ReferralBonusDB).filter(
        ReferralBonusDB.user_id == user_id
    ).all()
...
async def get_referral_stats(
    ...
    # Count stats
    total_codes = db.query(ReferralBonusDB).filter(
        ReferralBonusDB.user_id == user_id
    ).count()
    
    completed = db.query(ReferralBonusDB).filter(
        ReferralBonusDB.user_id == user_id,
        ReferralBonusDB.status == "applied"
    ).count()
    
    pending = db.query(ReferralBonusDB).filter(
        ReferralBonusDB.user_id == user_id,
        ReferralBonusDB.status == "pending"
    ).count()
