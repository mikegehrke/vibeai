...
class ProjectManager:
    ...
    def create_project(
        ...
        logger.info(
            "Created project %s for %s", project_id, user_email
        )
        ...
        for project_id in os.listdir(user_path):
            ...
            if os.path.isdir(project_dir):
                try:
                    ...
                    logger.warning(
                        "Failed to load project %s: %s", project_id, e
                    )
    ...
    def delete_project(self, user_email: str, project_id: str):
        ...
        if os.path.exists(project_path):
            ...
            logger.info(
                "Deleted project %s for %s", project_id, user_email
            )
        else:
    ...
    def _get_project_path(self, user_email: str, project_id: str) -> str:
        ...
        return os.path.join(
            self.base_path,
            self._sanitize_email(user_email),
            project_id
        )
    ...
    def _get_metadata_path(self, user_email: str, project_id: str) -> str:
        ...
        """
        return os.path.join(
            self._get_project_path(user_email, project_id),
            "project.json"
        )
