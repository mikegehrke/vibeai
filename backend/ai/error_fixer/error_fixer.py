...
class ErrorFixer:
            ...
            for pattern_info in patterns:
                ...
                if match:
                    ...
                    if 'file' in line.lower() or '/' in line or '\\' in line:
                        file_match = re.search(r'(?:File |at |in )?([^\s:]+\.(?:dart|js|jsx|ts|tsx|py|vue)):(\d+)(?::(\d+))?', line)
                        if file_match:
    ...
    def _fix_missing_dependency(self, error: ParsedError) -> Optional[CodeFix]:
        ...
        return CodeFix(
            file_path="package.json" if error.framework in [Framework.NODEJS, Framework.REACT] else "requirements.txt",
            line_number=1,
            original_code="",
    ...
    def _fix_type_error(self, error: ParsedError, file_content: Optional[str]) -> Optional[CodeFix]:
        ...
        match = re.search(r"Type '(\w+)' (?:is not assignable to|can't be assigned to) (?:type |parameter type )?'(\w+)'", error.message)
        if not match:
            return None
