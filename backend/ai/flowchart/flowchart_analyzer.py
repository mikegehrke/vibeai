...
class FlowchartAnalyzer:
    ...
    def _check_auth_barriers(self):
        ...
        for edge in self.edges:
            ...
            if (from_s.auth_level == AuthLevel.PUBLIC and
                to_s.auth_level == AuthLevel.AUTH_REQUIRED and
                not edge.requires_auth):
    ...
    def _check_logout_flow(self):
        ...
        has_logout = any(
            "logout" in edge.to_screen.lower() or
            "signout" in edge.to_screen.lower()
            for edge in self.edges
        )
        
        has_auth_screens = any(
            s.auth_level == AuthLevel.AUTH_REQUIRED
        ...
        if has_auth_screens and not has_logout:
            self.issues.append(FlowIssue(
                ...
                fix_data={
                    ...
                    "from_screens": [
                        s.name for s in self.screens.values()
                        if s.auth_level == AuthLevel.AUTH_REQUIRED
                    ]
                }
            ))
    ...
    def _check_error_handling(self):
        ...
        has_error_screen = any(
            "error" in s.name.lower() or
            "notfound" in s.name.lower() or
            "404" in s.name
            for s in self.screens.values()
        )
    ...
    def _check_payment_recovery(self):
        ...
        payment_screens = [
            s for s in self.screens.values()
            if "payment" in s.name.lower() or "checkout" in s.name.lower()
        ]
        
        for screen in payment_screens:
            has_failure_path = any(
                e.from_screen == screen.name and
                ("error" in e.to_screen.lower() or
                 "failure" in e.to_screen.lower() or
                 "retry" in e.to_screen.lower())
                for e in self.edges
            )
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "auth_required_screens": len([
                s for s in self.screens.values()
                if s.auth_level == AuthLevel.AUTH_REQUIRED
            ]),
            "public_screens": len([
                s for s in self.screens.values()
                if s.auth_level == AuthLevel.PUBLIC
            ]),
            "modal_screens": len([
                s for s in self.screens.values()
                if s.screen_type == ScreenType.MODAL
            ]),
            "tab_screens": len([
                s for s in self.screens.values()
                if s.tabs
    ...
    def _generate_suggestions(self) -> List[str]:
        ...
        if metrics["auto_fixable_issues"] > 0:
            suggestions.append(
                f"Apply auto-fix for {metrics['auto_fixable_issues']} issues"
            )
    ...
    def _fix_add_screen(self, fix_data: Dict) -> Dict[str, Any]:
        ...
        new_screen = Screen(
            ...
            route=f"/{self._camel_to_kebab(screen_name)}",
            auth_level=(
                AuthLevel.AUTH_REQUIRED if category in ["profile", "settings"]
                else AuthLevel.PUBLIC
            )
        )
    ...
    def _fix_add_auth_guard(self, fix_data: Dict) -> Dict[str, Any]:
        ...
        for edge in self.edges:
            if (edge.from_screen == edge_data["from"] and
                edge.to_screen == edge_data["to"]):
        ...
        return {
            "success": True,
            "message": "Added auth guard to navigation"
        }
    
    def _fix_add_logout_flow(self, fix_data: Dict) -> Dict[str, Any]:
        """Fix: Add logout flow"""
        # Add logout screen
        logout_screen = Screen(
            name="LogoutScreen",
            route="/logout",
    ...
    def export_to_mermaid(self) -> str:
        ...
        for edge in self.edges:
            ...
            lines.append(
                f"    {edge.from_screen} {arrow}{label} {edge.to_screen}"
            )
