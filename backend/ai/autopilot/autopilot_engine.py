...
# -------------------------------------------------------------
from ai.team.team_engine import team_engine  # Re-enabled import
import logging
import os
...
class AutopilotEngine:
    ...
    async def build_feature(
        self, user_id: str, project_id: str, task: str, auto_deploy: bool = False
    ) -> Dict[str, Any]:
        """
...
Erstelle ALLE benötigten Dateien mit komplettem Code."""
        ...
        code_gen = await agent_dispatcher.dispatch(task=code_task, agent_type="lead_developer", quality=9)
...
{test_strategy[:500]}"""
        ...
        test_gen = await agent_dispatcher.dispatch(task=test_task, agent_type="test_engineer", quality=7)
...
- Type Errors"""
        ...
        error_check = await agent_dispatcher.dispatch(task=error_check_task, agent_type="error_fixer", quality=8)
...
{code_gen.get('response', '')[:2000]}"""
            ...
            fix_result = await agent_dispatcher.dispatch(task=fix_task, agent_type="error_fixer", quality=8)
    ...
    async def optimize_project(self, user_id: str, project_id: str) -> Dict[str, Any]:
        """
                ...
                if file.endswith((".py", ".js", ".ts", ".dart", ".java")):
                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            code_files.append({"path": file_path, "content": f.read()[:5000]})  # Limit
                    except:
                        pass
