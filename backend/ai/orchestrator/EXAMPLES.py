...
async def example_full_workflow():
    ...
    result = await orchestrator.execute_workflow(
        ...
        workflow="full_cycle",
        params={
            "prompt": "Create a social media app with feed and profile"
        }
    )
...
async def example_multi_framework():
    ...
    ui_result = await orchestrator.handle(
        user_id="user123",
        project_id="proj456",
        prompt="Create a settings screen"
    )
...
async def example_context_management():
    ...
    project_context.update(
        "user123",
        "proj456",
        {"framework": "react", "custom_field": "value"}
    )
...
async def example_end_to_end():
    ...
    # Update context
    project_context.set_project_path(
        user_id,
        project_id,
    ...
    code_result = await orchestrator.handle(
        user_id=user_id,
        project_id=project_id,
        prompt="Generate Flutter code"
    )
    ...
    preview_result = await orchestrator.handle(
        user_id=user_id,
        project_id=project_id,
        prompt="Show preview"
    )
    ...
    print("\n=== STEP 5: Build APK ===")
    build_result = await orchestrator.handle(
        user_id=user_id,
        project_id=project_id,
        prompt="Build APK"
    )
    ...
    print("\n=== STEP 6: Deploy ===")
    deploy_result = await orchestrator.handle(
        user_id=user_id,
        project_id=project_id,
        prompt="Deploy app"
    )
