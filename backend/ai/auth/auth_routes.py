...
async def generate_auth_backend(request: Request, data: GenerateBackendRequest):
    ...
    try:
        ...
        result = auth_generator.generate_backend(
...
async def generate_auth_frontend(request: Request, data: GenerateFrontendRequest):
    ...
    try:
        ...
        result = auth_generator.generate_frontend_ui(
            framework=data.framework,
            base_path=base_path,
            style=data.style
        )
...
async def generate_full_auth(request: Request, backend_framework: str, frontend_framework: str, project_id: str):
    ...
    try:
        ...
        # Backend generieren
        backend_result = auth_generator.generate_backend(
        ...
        # Frontend generieren
        frontend_result = auth_generator.generate_frontend_ui(
            framework=frontend_framework,
            base_path=base_path
        )
        
        if not frontend_result.get("success"):
            raise HTTPException(status_code=400, detail=f"Frontend Fehler: {frontend_result.get('error')}")
...
async def get_auth_template(framework: str, type: str):
    ...
    return {
        "framework": framework,
        "type": type,
        "preview": templates[framework][type]
    }
