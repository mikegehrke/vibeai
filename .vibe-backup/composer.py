...
Output a clean, unified response that combines the best of all agent contributions."""
    ...
    async def synthesize_agent_outputs(
        ...
        if not agent_outputs:
            return {"status": "error", "error": "No agent outputs to synthesize"}
    ...
    async def assemble_project(
        self, project_plan: Dict, worker_outputs: Dict[str, Dict], output_directory: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for relative_path, content in base_structure.items():
                file_path = self._write_file(output_directory, relative_path, content)
            ...
            for module_name, module_data in worker_outputs.items():
                ...
                for file_name, file_content in module_data.items():
                    file_path = self._write_file(module_dir, file_name, file_content)
                    written_files.append(file_path)

            # 3. Generate README
            readme_path = self._generate_readme(output_directory, project_name, project_type, project_plan)
            written_files.append(readme_path)

            # 4. Generate manifest/config files
            config_files = self._generate_config_files(output_directory, project_type, project_plan)
    ...
    def _write_file(self, base_directory: str, relative_path: str, content: str) -> str:
        """
...
"""
    ...
    def _generate_config_files(self, output_directory: str, project_type: str, project_plan: Dict) -> List[str]:
        """
        ...
        if gitignore_content:
            config_files.append(self._write_file(output_directory, ".gitignore", gitignore_content))
        ...
        if project_type in ["react", "nextjs", "nodejs"]:
            ...
            config_files.append(self._write_file(output_directory, "package.json", package_json))

        # Generate pubspec.yaml for Flutter projects
        if project_type == "flutter":
            pubspec = self._generate_pubspec_yaml(project_plan)
            config_files.append(self._write_file(output_directory, "pubspec.yaml", pubspec))
...
async def synthesize_outputs(agent_outputs: List[Dict], context: Optional[Dict] = None) -> Dict:
    """
...
async def build_project(project_plan: Dict, worker_outputs: Dict, output_dir: str) -> Dict:
    """
