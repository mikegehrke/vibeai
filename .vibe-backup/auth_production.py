...
def check_rate_limit(identifier: str, max_attempts: int = 5, window_seconds: int = 300):
    ...
    if USE_REDIS and redis_client:
        ...
        # Remove old attempts
        login_attempts[identifier] = [
            attempt for attempt in login_attempts[identifier] if now - attempt < window_seconds
        ]
...
async def get_user_from_api_key(
    api_key: str = Header(None, alias="X-API-Key"), db: Session = Depends(get_db)
) -> Optional[User]:
    """
...
async def verify_google_token(token: str) -> Optional[Dict]:
    ...
    try:
        ...
        resp = await httpx.AsyncClient().get(f"https://oauth2.googleapis.com/tokeninfo?id_token={token}", timeout=5)
...
async def register(data: RegisterRequest, request: Request, db: Session = Depends(get_db)):
    ...
    create_verification_token(data.email)
...
async def logout(
    ...
    # Audit log
    log_auth_event(db, user_id=current_user.id, event_type="logout", success=True)
...
async def confirm_password_reset(data: PasswordResetConfirm, db: Session = Depends(get_db)):
    ...
    # Audit log
    log_auth_event(db, user_id=user.id, event_type="password_reset", success=True)
...
async def enable_2fa(
    ...
    # Audit log
    log_auth_event(db, user_id=current_user.id, event_type="2fa_enabled", success=True)
...
async def disable_2fa(
    ...
    # Audit log
    log_auth_event(db, user_id=current_user.id, event_type="2fa_disabled", success=True)

    return {"message": "2FA disabled successfully"}


@router.post("/2fa/verify", response_model=TokenResponse)
async def verify_2fa(user_id: int, code: str, db: Session = Depends(get_db)):
    """
...
async def get_active_sessions(admin: User = Depends(require_admin)):
    ...
    """
    sessions = [{"key": key, "data": data} for key, data in session_store.items() if key.startswith("session:")]
    return {"total": len(sessions), "sessions": sessions}


@router.delete("/admin/sessions/{user_id}")
async def revoke_user_sessions(user_id: int, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    """
