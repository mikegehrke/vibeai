...
class ArchitecturePlannerV2:
    ...
    async def plan_app(
        ...
        if not model:
            model_name = model_router_v2.pick_model(description, agent="devra")  # Deep reasoning für Planning
        else:
            model_name = model
...
"""
    ...
    async def estimate_project(self, plan: Dict, model: Optional[str] = None) -> Dict:
        """
