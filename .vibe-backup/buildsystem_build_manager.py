...
class BuildManager:
    ...
    async def create_build(
        ...
        logger.info("Created build %s for user %s", build_id, user_email)
    ...
    async def update_build_status(self, build_id: str, status: BuildStatus, error: Optional[str] = None):
        """
        Update build status.
        """
        if build_id not in self.active_builds:
    ...
    async def add_artifact(self, build_id: str, artifact_type: str, file_path: str, size: int):
        """
    ...
    def list_builds(self, user_email: str, limit: int = 50) -> List[Dict]:
        """
        List user builds.
        """
        user_builds = [b for b in self.active_builds.values() if b["user"] == user_email]

        # Sort by created_at descending
        user_builds.sort(key=lambda x: x["created_at"], reverse=True)
    ...
    def _get_metadata_path(self, build_id: str) -> str:
        ...
        return os.path.join(self._get_build_path(build_id), "build.json")
