...
async def stop_preview(request: StopPreviewRequest):
    ...
    try:
        result = await unified_preview_manager.stop_preview(user_id=request.user_id, project_id=request.project_id)
...
async def reload_preview(request: ReloadPreviewRequest):
    ...
    try:
        result = await unified_preview_manager.reload_preview(user_id=request.user_id, project_id=request.project_id)

        if not result.get("success"):
            raise HTTPException(status_code=500, detail=result.get("error", "Failed to reload preview"))
