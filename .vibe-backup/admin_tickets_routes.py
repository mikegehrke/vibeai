...
async def create_ticket_v2(payload: TicketCreateV2, db=Depends(get_db)):
    """
    ...
    """
    # AI Analysis
    analysis = await ai_ticket_assistant.analyze_ticket(subject=payload.subject, message=payload.message)
...
async def ai_suggest_response(ticket_id: str, db=Depends(get_db), _=Depends(require_admin)):
    """
    ...
    history = [{"role": "user", "content": f"Subject: {ticket.subject}\n\n{ticket.message}"}]
...
async def bulk_analyze_tickets(ticket_ids: List[str], db=Depends(get_db), _=Depends(require_admin)):
    """
    ...
    for ticket_id in ticket_ids:
        ...
        analysis = await ai_ticket_assistant.analyze_ticket(subject=ticket.subject, message=ticket.message)
