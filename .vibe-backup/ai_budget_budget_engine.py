...
class BudgetEngine:
    ...
    def set_budget(self, user_id: str, period: BudgetPeriod, limit_euros: float):
    ...
    def add_cost(
        ...
        # Calculate cost
        cost = self.pricing_db.calculate_cost(model_id=model_id, input_tokens=input_tokens, output_tokens=output_tokens)
    ...
    def get_spend(self, user_id: str, period: BudgetPeriod = BudgetPeriod.DAILY) -> float:
    ...
    def get_remaining(self, user_id: str, period: BudgetPeriod = BudgetPeriod.DAILY) -> Optional[float]:
    ...
    def estimate_cost(self, model_id: str, input_tokens: int, output_tokens: int) -> float:
        ...
        return self.pricing_db.calculate_cost(model_id=model_id, input_tokens=input_tokens, output_tokens=output_tokens)
        ...
        for period in BudgetPeriod:
            ...
            if budget:
                status[period.value] = {
                    ...
                    "percentage_used": (
                        (budget.current_spend / budget.limit_euros * 100) if budget.limit_euros > 0 else 0
                    ),
    ...
    def get_total_spent(self, user_id: str) -> float:
        ...
        return sum(tx.cost_euros for tx in self.transactions if tx.user_id == user_id)

    def downgrade_model_if_needed(self, user_id: str, current_model: str, min_quality: int = 5) -> str:
        """
        ...
        if remaining < 0.01:
            return self.pricing_db.get_cheapest_model(quality_min=min_quality, capabilities=None)

        # If budget is tight, find cheaper model
        if remaining < 0.05 and avg_price > 0.003:
            cheaper = self.pricing_db.get_cheapest_model(quality_min=min_quality, capabilities=None)
            return cheaper

        return current_model

    def _get_budget(self, user_id: str, period: BudgetPeriod) -> Optional[BudgetLimit]:
...
if __name__ == "__main__":
    ...
    status = budget_engine.get_budget_status("user123")
    print(
        f"   Daily: €{status['daily']['spent']:.4f} / €{status['daily']['limit']:.2f} ({status['daily']['percentage_used']:.1f}%)"
    )
