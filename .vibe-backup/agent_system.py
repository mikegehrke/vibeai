        ...
        try:
            ...
            if ws_manager_v2:
                await ws_manager_v2.notify_code_studio_event(
                    event=f"{action}_completed", message=f"Code {action} completed"
                )
            ...
            return {"agent": "code_engineer", "error": str(e), "status": "failed"}
...
class SearchAgent:
    ...
    async def run(self, query: str, search_type: str = "web", max_results: int = 5):
        """
        Perform search.
        ...
        try:
            ...
            if search_type == "web":
                results = await searcher.search_web(query=query, num_results=max_results)
            elif search_type == "news":
                results = await searcher.search_news(query=query, max_results=max_results)
            else:
                results = await searcher.search_web(query=query)

            # Analyze results with AI
            from core.model_router_v2 import model_router

            analysis = await model_router.route_request(
                messages=[
                    {
                        "role": "user",
            ...
            return {"agent": "search", "error": str(e), "status": "failed"}
...
class MemoryAgent:
    ...
    async def run(self, action: str, user_id: str, agent_name: Optional[str] = None, **kwargs):
        """
        Memory operations.
        ...
        try:
            if action == "get_history":
                ...
                return {"agent": "memory", "action": "add_message", "status": "success"}

            elif action == "set_preference":
                self.context_memory.set_preference(user_id=user_id, key=kwargs.get("key"), value=kwargs.get("value"))
                return {
                    "agent": "memory",
                ...
                self.context_memory.clear_history(user_id=user_id, agent_name=agent_name)
                return {
                    "agent": "memory",
            ...
            return {"agent": "memory", "error": str(e), "status": "failed"}
    ...
    async def run(self, task: str, context: Optional[Dict] = None):
        ...
        try:
            response = await model_router.route_request(
                ...
                agent_name="devra",  # Reasoning agent
                user_id="system",
                preferred_model="o3",
            )
            ...
            return {"agent": "planner", "task": task, "plan": plan, "status": "success"}

        except Exception as e:
            return {"agent": "planner", "error": str(e), "status": "failed"}
...
class WorkerAgent:
    ...
    async def run(self, step: Dict, context: Optional[Dict] = None):
        """
